...
from django.contrib.auth.models import Permission as ps
...
def check_location(req):
    location = req.get_host().split(":")[0]
    if location == "127.0.0.1" or location == "localhost":
        
        return True
    else:
        print("Unauthorized access, Unable to initialize data!")
        return False


def main(request):
    if check_location(request):
        if ps.objects.all().exists():
            url.SetUrlNameDatabaseList(request)
            permission.AddPermission(request)
            setModels_id(request)
            setDefaultUser(request)
            setEventType(request)
            setCrojob_inSolrData(request)
            setResumeCustomLabel(request)
            setSettings(request)
    return HttpResponse("")
...
def setResumeCustomLabel(request):
    try:
