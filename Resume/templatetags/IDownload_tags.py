...
@register.simple_tag
def SetIDownloadTable(data):


	_view_list = ["邮箱", "电话", "下载时间", "申请岗位", "下载类型", "简历状态"]
	view_list = ["username", "email", "phone", "download_time", "jobs", "resume_status", "id", "number"]
	obj = data.values()
	ele = ''
	ele += '<thead><tr>'
	...
	for item in obj:
		...
		ele += '<td>{}</td>'.format((item["download_time"] + datetime.timedelta(hours=+8)).strftime('%y/%m/%d %H:%M'))
		ele += '<td>{}</td>'.format(sub_obj.jobs)
		ele += '<td>{}</td>'.format(getDownloadTypeChoicesValue(item["download_type"]))
