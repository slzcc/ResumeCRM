...
@register.simple_tag
def SetIamTrackingTable(data):
	_view_list = ["邮箱", "电话", "修改时间", "申请岗位", "简历状态"]
	view_list = ["username", "email", "phone", "modify_time", "jobs", "resume_status", "id"]
	...
	for item in obj:
		...
		ele += '<td>{}</td>'.format((item["modify_time"] + datetime.timedelta(hours=+8)).strftime('%y/%m/%d %H:%M'))
